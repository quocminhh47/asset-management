...
public class RequestReturningController {
    ...
    public RequestReturningController(RequestReturningService requestReturningService) {
        this.requestReturningService = requestReturningService;
    }
    ...
    @Operation(summary = "Cancel request returning",
            description = "Admin want to cancel returning request, so that admin can " +
                    "remove" +
                    " request form request list when necessary it allows to cancel only" +
                    " “Waiting for returning” requests")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK - Successfully "),
            @ApiResponse(responseCode = "400",
                    description = "Bad Request - The request is invalid"),
            @ApiResponse(responseCode = "401", description = "UNAUTHORIZED - The request is unauthorized"),
            @ApiResponse(responseCode = "403", description = "FORBIDDEN - You don’t have permission to access"),
            @ApiResponse(responseCode = "404", description = "NOT FOUND - The request " +
                    "resource is not found"),
            @ApiResponse(responseCode = "406", description = "NOT_ACCEPTABLE - The " +
                    "state of the request is 'Completed'"),
            @ApiResponse(responseCode = "500",
                    description = "Internal Error - There were some error while processing in server")
    })
    @DeleteMapping("/{id}")
