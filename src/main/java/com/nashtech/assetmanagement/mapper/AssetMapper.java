...
public class AssetMapper {
	...
	public List<AssetState> mapperListStates(List<String> list) {
		List<AssetState> result = list.stream().map(item -> mapper.map(item, AssetState.class))
				.collect(Collectors.toList());
		return result;
	}

	public ListSearchingAssetResponseDto getAssetListToResponseAssetDTOList(List<Asset> assetList) {
		List<AssetResponseDto> responseList = assetList.stream().map(asset -> mapper.map(asset, AssetResponseDto.class))
				.collect(Collectors.toList());
		ListSearchingAssetResponseDto responseDto = new ListSearchingAssetResponseDto(responseList.size(),
				responseList);
		return responseDto;
	}
	...
	public Asset mapEditAssetRequestToEntity(EditAssetRequestDto request, Asset asset) {
		try {
			...
			if (installedDate.after(dateNow))
				throw new DateInvalidException("Date.is.must.before.today:" + dateNow.toString().replaceAll(" ", "."));
	...
	public AssetReportResponseDto mapToAssetReportDto(Page<IAssetReportResponseDto> iAssetPage) {
		...
		return AssetReportResponseDto.builder().assetContent(assetReportContent).pageNo(iAssetPage.getNumber())
				.pageSize(iAssetPage.getSize()).totalElements(iAssetPage.getTotalElements())
				.totalPages(iAssetPage.getTotalPages()).last(iAssetPage.isLast()).build();
